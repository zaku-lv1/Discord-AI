...
2. 管理パネルにアクセス:
   ...
   - **ローカルアカウント**でログイン（新規登録も可能）または「Discordでログイン」をクリックして認証

## 🎯 使用方法

### 認証システム

このシステムは2つの認証方法をサポートしています：

#### ローカルアカウント認証
- **新規登録**: ユーザー名、パスワード、メールアドレス（任意）で新しいアカウントを作成
- **ログイン**: 作成したユーザー名とパスワードでログイン
- **セキュリティ**: bcryptを使用した安全なパスワードハッシュ化（12saltラウンド）
- **検証**: ユーザー名は3文字以上の英数字、アンダースコア、ハイフンのみ
- **パスワード**: 6文字以上の安全なパスワード

#### Discord OAuth認証
- **既存システム**: 従来のDiscord OAuthログインも引き続き利用可能
- **アバター表示**: Discordアバターの自動表示
- **プロファイル連携**: Discordプロファイル情報の自動取得

両方の認証方法は同時に利用でき、ユーザーは好みに応じて選択できます。
...
1. **ログイン**: 以下の方法で認証
   - **ローカルアカウント**: ユーザー名とパスワードで直接ログイン
   - **Discord OAuth**: Discordアカウントを使用して認証
